...
for h in range(4):    
    ...
    with open('KNN_trials_t+'+str(h+1)+'.pkl', 'rb') as f:
    ...
    for k in range(len(espiras)):        
        ...
        model = KNeighborsRegressor(n_neighbors=int(np.array(params['vecinos'])),
                                    weights=pesos[int(np.array(params['weights']))],
                                    p=p[int(np.array(params['p']))])
    ...
    with open('KNN_metrics_t+'+str(h+1)+'.pkl', 'wb') as f:
