...
for h in range(4):    
    ...
    with open('RFR_trials_t+'+str(h+1)+'.pkl', 'rb') as f:
    ...
    for k in range(len(espiras)):        
        ...
        model = RandomForestRegressor(n_estimators=estimators[int(np.array(params['estimators']))],
    ...
    with open('RFR_metrics_t+'+str(h+1)+'.pkl', 'wb') as f:
