...
# -*- coding: utf-8 -*-
"""
Created on Tue May 26 16:34:03 2020
...
def objective(params):        
    ...
    for i in range(3):
        #initialize
        model = AdaBoostRegressor(n_estimators=int(params['estimators']), loss=params['loss'], learning_rate=params['lr'])
        #model train
...
for h in range(4):    
    ...
    for k in range(len(espiras)):
        ...
        if h==0:
         ...
         # Parameter Space
        SPACE_FNN = dict([('lr',  hp.uniform('lr',0.1,1)),
    ...
    with open('ADA_trials_t+'+str(h+1)+'.pkl', 'wb') as f:
