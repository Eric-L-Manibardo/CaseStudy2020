...
def objective(params):        
    ...
    for i in range(3):
        #initialize
        model = KNeighborsRegressor(n_neighbors=int(params['vecinos']), weights=params['weights'], p=params['p'])
        #model train
...
for h in range(4):    
    ...
    for k in range(len(espiras)):
        ...
        if h==0:
         ...
         # Parameter Space
        SPACE_FNN = dict([('vecinos',  hp.quniform('vecinos',1,200,1)),
        ...
        best = fmin(objective, SPACE_FNN, algo=tpe.suggest, trials =trials, max_evals=100)
        print(best)
        pruebas.append(trials)
    
    with open('KNN_trials_t+'+str(h+1)+'.pkl', 'wb') as f:
