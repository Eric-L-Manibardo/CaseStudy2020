...
for h in range(4):    
    ...
    with open('GBR_trials_t+'+str(h+1)+'.pkl', 'rb') as f:
	...
	X_test = StandardScaler().fit_transform(df_test.iloc[:,1:6].values)
        ...
        model = GradientBoostingRegressor(n_estimators=estimators[int(np.array(params['estimators']))],
    ...
    with open('GBR_metrics_t+'+str(h+1)+'.pkl', 'wb') as f:
