...
for h in range(4):        
    ...
    for k in range(len(espiras)):        
        ...
        if h==0:
         ...
         # Parameter Space
        SPACE_FNN = dict([('do',  hp.quniform('h1',10,100,1)),
                      ('not',  hp.quniform('h2',10,100,1)),
                      ('used',  hp.quniform('h3',10,100,1))
            ])
        trials = Trials()
            
        #Try to minimize MSE over different configurations
        best = fmin(objective, SPACE_FNN, algo=tpe.suggest, trials =trials, max_evals=1)
        print(best)
        pruebas.append(trials)
    
    with open('LR_trials_t+'+str(h+1)+'.pkl', 'wb') as f:
